/* =========================================================
   ...
   Motor de apresentação comum às seis aulas — navegação, passos,
   cronômetro, notas do docente, visão geral, impressão.
   Herdado sem alteração de comportamento do deck de Análise
   Estratégica (exemplo/deck.js), com uma correção: history.replaceState
   envolto em try/catch, pois pode lançar SecurityError em file://.
   ========================================================= */
...
(function () {
    ...
    function go(i, mode) {
        ...
        if (location.hash !== '#' + (cur + 1)) {
            // file:// pode negar history.replaceState em alguns navegadores;
            // sem o try/catch, o SecurityError derrubaria a navegação inteira.
    ...
    // ajusta total e tempo restante juntos, para que R (zerar) volte ao
    // valor já corrigido em sala, não ao data-t original do slide.
    function bumpTimer(deltaSec) {
        var st = timerState();
        if (!st) return;
        st.total = Math.max(0, st.total + deltaSec);
        st.left = Math.max(0, Math.min(st.total, st.left + deltaSec));
        paintTimer();
    }
    ...
    document.addEventListener('keydown', function (e) {
        ...
        switch (k.toLowerCase()) {
            ...
            case '+': case '=': bumpTimer(30); break;
            case '-': bumpTimer(-30); break;
